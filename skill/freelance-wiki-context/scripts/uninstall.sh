#!/usr/bin/env bash
# Uninstaller for the freelance-wiki-context Hermes skill — reverses exactly
# what scripts/setup.sh does, back to a clean state:
#
#   curl -fsSL https://raw.githubusercontent.com/idjugostran/freelance-wiki/main/skill/freelance-wiki-context/scripts/uninstall.sh | bash
#
# What it does, in order:
#   0. Delete the installed checkout (default ~/Freelance) — the wiki/ +
...
# (those may still mention Upwork/Fiverr from past replies), and it never
# touches the wiki's source repo (github.com/idjugostran/freelance-wiki) or
# any dev clone of it — only the installed copy on *this* machine.
#
...
#   FREELANCE_INSTALL_DIR   dir to remove (default: ~/Freelance — must match
#                            whatever --dir/FREELANCE_INSTALL_DIR setup.sh
...
# Only delete if it actually looks like a freelance-wiki checkout, so a
...
elif [[ ! -d "$INSTALL_DIR/.git" ]] || [[ ! -d "$INSTALL_DIR/skill/freelance-wiki-context" ]]; then
  echo "  WARNING: $INSTALL_DIR doesn't look like a freelance-wiki checkout" >&2
  echo "           (missing .git or skill/freelance-wiki-context) - refusing to delete it." >&2
