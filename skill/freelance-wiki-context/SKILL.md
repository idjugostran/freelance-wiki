...
3. **Synthesize an answer grounded in what you read, citing in chat-safe
   ...
   - **Disclose that this skill answered.** End the reply with a short,
     plain-text marker on its own line, e.g. `📚 Источник: Freelance Wiki
     (freelance-wiki-context)` — no brackets/links, just text, so it can't
     trip the same Markdown-parsing issue as citations above.
...
confirm the reply ends with the `📚 Источник: Freelance Wiki
(freelance-wiki-context)` disclosure line — if it's missing, add it before
sending.
